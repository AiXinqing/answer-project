...
<script>
  ...
  export default {
    data() {
      return {
        ...
        parameter:{
          ...
          minScore:'',
          maxScore:'',
          url:this.URL.GetClassSubjectScoreCriticalityStuDetails
        },
        page: {
          pageSize: 15,
          pageNum: 1,
    ...
    computed: {
      ...
      ...mapState('criticalBirthDetail', ['tableLoading','TableList','pagination']),
    ...
    methods: {
      ...
      getTable() {
        ...
        this.$store.dispatch('criticalBirthDetail/GetStuResults', this.parameter)
      },

      downTable(){
        // 下载表格
        const {cid,tid,maxScore,minScore,tsid} = this.parameter
        window.open(`${this.URL.ExportClassSubjectScoreCriticalityStuDetails}?tid=${tid}&minScore=${minScore}&cid=${cid}&maxScore=${maxScore}&tsid=${tsid}`)
      },
    },
  }
</script>
